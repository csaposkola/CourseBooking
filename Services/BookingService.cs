...
{
    // Custom Exception for Booking Logic Errors
    ...
    // Using ServiceLocator pattern requires careful context management or explicit DI
    public class BookingService : ServiceLocator<IBookingService, BookingService>, IBookingService
    {
        private readonly IDataContext _dataContext;
        private readonly Lazy<ModuleSettingsRepository> _settingsRepoLazy = new Lazy<ModuleSettingsRepository>(() => new ModuleSettingsRepository());
        private const int DefaultCancellationHours = 24;

        protected int ModuleId { get; set; } = -1;
        protected int PortalId { get; set; } = -1;
        private ModuleSettingsRepository SettingsRepo => _settingsRepoLazy.Value;

        // Default constructor for ServiceLocator
        public BookingService()
        {
            _dataContext = DataContext.Instance(); // Get PetaPoco context
        }

        // Constructor for direct instantiation (preferred if not using ServiceLocator strictly)
        public BookingService(int moduleId, int portalId) : this()
        {
            SetContextInternal(moduleId, portalId);
        }

        // Method to set context if instantiated via ServiceLocator without parameters
        ...
        // Helper to ensure context is set before use
        private void EnsureContext()
        {
            if (this.ModuleId < 0 || this.PortalId < 0)
            {
                // Fallback attempt - use with caution
        ...
        private T GetSetting<T>(string key, T defaultValue)
        {
            ...
            if (moduleInfo == null) { LogWarn($"ModuleInfo null for MId {this.ModuleId}. Using default for '{key}'."); return defaultValue; }
            return SettingsRepo.GetSetting(moduleInfo, key, defaultValue); // Uses ModuleSettingsRepository
        }
        ...
        // *** FIX: Helper to check for positive IDs ***
        private void CheckPositiveId(string paramName, int id)
        {
            Requires.NotNegative(paramName, id); // Ensure not negative first
        ...
        public IEnumerable<CoursePlanEntity> GetCoursePlans(bool includeNonPublic = false)
        {
            ...
            try {
                ...
                // Assuming Scope handles ModuleId filtering. Add explicit WHERE ModuleId = @0 if needed.
        ...
        {
            EnsureContext();
            CheckPositiveId(nameof(coursePlanId), coursePlanId); // *** FIX: Use helper ***
            try {
        ...
        // Helper for efficient fetching by multiple IDs
        private IDictionary<int, CoursePlanEntity> GetCoursePlansByIds(IDataContext ctx, List<int> planIds)
        {
            ...
            try {
                // Add explicit `AND ModuleId = @1` filter if Scope isn't reliable
        ...
        public IEnumerable<CourseScheduleEntity> GetCourseSchedules(DateTime? fromDate = null, DateTime? toDate = null, bool includeInactive = false)
        {
            EnsureContext();
            try {
                // Explicit ModuleId filter is safer
                ...
                if (schedules.Any()) {
                    ...
                    var bookingCounts = GetBookingCountsForSchedules(_dataContext, scheduleIds); // Uses corrected ExecuteQuery
        ...
        {
            EnsureContext();
            CheckPositiveId(nameof(scheduleId), scheduleId); // *** FIX: Use helper ***
            try {
                ...
                if (schedule != null) {
                    ...
                    schedule.BookingCount = GetBookingCountForSchedule(schedule.ID); // Uses correct ExecuteScalar
                }
                return schedule;
        ...
        public CourseScheduleEntity CreateCourseSchedule(CourseScheduleEntity schedule)
        {
            ...
            CheckPositiveId(nameof(schedule.CoursePlanID), schedule.CoursePlanID); // *** FIX: Use helper ***
            try {
                ...
                schedule.ModuleId = this.ModuleId; schedule.CreatedDate = DateTime.UtcNow;
        ...
        {
            EnsureContext(); Requires.NotNull("schedule", schedule);
            CheckPositiveId(nameof(schedule.ID), schedule.ID); // *** FIX: Use helper ***
            try {
                ...
                schedule.ModuleId = this.ModuleId; // Preserve ModuleId
                if (schedule.StartTime.Kind != DateTimeKind.Utc) { LogWarn($"Non-UTC StartTime ({schedule.StartTime.Kind}), converting."); schedule.StartTime = schedule.StartTime.ToUniversalTime(); }
        ...
        {
            EnsureContext(); CheckPositiveId(nameof(scheduleId), scheduleId); // *** FIX: Use helper ***
            try {
                var schedule = GetCourseScheduleById(scheduleId);
                if (schedule == null) return false;
                schedule.IsActive = false; // Soft delete (mark inactive)
        ...
        {
            EnsureContext(); CheckPositiveId(nameof(userId), userId); // *** FIX: Use helper ***
            try {
        ...
        {
            EnsureContext(); CheckPositiveId(nameof(bookingId), bookingId); // *** FIX: Use helper ***
            try {
        ...
        {
            EnsureContext(); CheckPositiveId(nameof(courseScheduleId), courseScheduleId); CheckPositiveId(nameof(userId), userId); // *** FIX: Use helpers ***
            try {
                ...
                var booking = new BookingEntity {
                    ModuleId = this.ModuleId, CourseScheduleID = courseScheduleId, UserID = userId, BookingTime = DateTime.UtcNow, IsCancelled = false,
                    VoucherCode = GenerateVoucherCode(), PaymentStatus = "Pending", Notes = notes?.Length > 500 ? notes.Substring(0, 500) : notes
                };
        ...
        // Helper for efficient fetching of booking counts
        private IDictionary<int, int> GetBookingCountsForSchedules(IDataContext ctx, List<int> scheduleIds)
        {
             ...
             try {
                // *** FIX: Corrected ExecuteQuery call with CommandType.Text FIRST ***
                var results = ctx.ExecuteQuery<dynamic>(
                    CommandType.Text, // Explicitly specify CommandType
        ...
        public int GetBookingCountForSchedule(int scheduleId)
        {
            EnsureContext(); CheckPositiveId(nameof(scheduleId), scheduleId); // *** FIX: Use helper ***
            try {
                // ExecuteScalar does NOT take CommandType first
        ...
        {
            EnsureContext(); CheckPositiveId(nameof(scheduleId), scheduleId); CheckPositiveId(nameof(userId), userId); // *** FIX: Use helpers ***
            try {
                 // ExecuteScalar does NOT take CommandType first
                return _dataContext.ExecuteScalar<int>(
        ...
        // LoadBookingDetails remains the same complex logic, ensure dependencies are fetched correctly
        ...
        // GenerateVoucherCode remains the same
